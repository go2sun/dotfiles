package main
import (
    "bytes" "encoding/base64" "encoding/json" "flag" "fmt"
    "net/http" "os" "os/exec" "path/filepath" "strings" "time"
)
const (
...
func trigger() {
    ...
    payload, _ := json.Marshal(map[string]interface{}{
        "model": VisionModel, "prompt": "请分析此屏幕。识别任何技术故障(BUG)、核心重点(FOCUS)和关键词(KEYWORDS)。必须使用中文输出。格式：BUG: [描述或无], FOCUS: [描述], KEYWORDS: [关键词列表]", "stream": false, "images": []string{imgBase64},
    })
    resp, _ := http.Post(OllamaURL, "application/json", bytes.NewBuffer(payload))
    var res struct{ Response string }; json.NewDecoder(resp.Body).Decode(&res)
